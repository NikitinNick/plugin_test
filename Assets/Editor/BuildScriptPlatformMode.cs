...
namespace UnityEditor.AddressableAssets.Build.DataBuilders
{
    ...
    [CreateAssetMenu(fileName = "BuildScriptPlatform.asset",
        menuName = "Addressables/Content Builders/Build Script For Platform")]
    public class BuildScriptPlatformMode : BuildScriptBase
    {
        public string TestField;
        /// <inheritdoc />
        public override string Name => "Build Script For Platform";
        ...
        Dictionary<string, string> m_BundleToInternalId = new();
            ...
            out AddressableAssetsBuildContext aaContext)
        {
            ...
            var runtimeData = new ResourceManagerRuntimeData
            {
                ...
                AddressablesVersion =
                    PackageManager.PackageInfo.FindForAssembly(typeof(Addressables).Assembly)?.version,
#endif
            ...
            AddressableAssetsBuildContext aaContext) where TResult : IDataBuilderResult
        {
            ...
            using (Log.ScopedStep(LogLevel.Info, "Generate Catalog"))
            {
                ...
                contentCatalog.SetData(aaContext.locations.OrderBy(f => f.InternalId).ToList(),
                    aaContext.Settings.OptimizeCatalogSize);
            ...
            AddressableAssetsBuildContext aaContext)
        {
           ...
           if (loadPath.StartsWith("http://") && PlayerSettings.insecureHttpOption == InsecureHttpOption.NotAllowed)
            ...
            var list = PrepGroupBundlePacking(assetGroup, bundleInputDefs, schema, EntryFilter);
        ...
        private static bool EntryFilter(AddressableAssetEntry entry)
        {
            ...
            var buildPlatform = PlatformMappingService.GetPlatformPathSubFolder().ToLowerInvariant();

            var labels = entry.labels;

            if (labels.Count > 0)
            {
                if (!labels.Contains(buildPlatform))
                {
                    result = false;
                }
            ...
            for (int i = 0; i < buildBundles.Count; ++i)
            {
                if (primaryKeyToCatalogEntry.TryGetValue(buildBundles[i], out ContentCatalogDataEntry dataEntry))
                {
                    ...
                    if (assetGroup == assetGroup.Settings.DefaultGroup && info.Dependencies.Length == 0 &&
                        ...
                        (info.FileName.EndsWith("_unitybuiltinshaders.bundle") ||
                         info.FileName.EndsWith("_monoscripts.bundle")))
                    {
        ...
        private static bool GroupFilter(AddressableAssetGroup g)
        {
            if (g == null)
                return false;
